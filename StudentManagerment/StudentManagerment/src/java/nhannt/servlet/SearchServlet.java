...
public class SearchServlet extends HttpServlet {
    private final String showSearchAdmin = "Admin.jsp";
    /**
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (searchValue == null || searchValue == "") {
                allSize = memberdao.countAllMember();
                request.setAttribute("ALLSIZE", allSize);
                memberdao.loadAllUser();
                List<MemberDTO> result = memberdao.getListUser();
                request.setAttribute("LISTMEMBER", result);
            } else if (searchValue.trim().length() > 0) {
                allSize = memberdao.countAllMember();
                request.setAttribute("ALLSIZE", allSize);
                memberdao.searchByLastFistName(searchValue, roleID);
                List<MemberDTO> result = memberdao.getListUser();
                request.setAttribute("LISTMEMBER", result);
            } 
        } catch (SQLException e) {
            log("Can not connect DBI in Search Servlet", e);
        } catch (ClassNotFoundException e) {
            log("Can not find Class in Search Servlet", e);
        } finally {
